...
def gps_driver(gps_data, serial_id, nmea_id):
    ...
    while not rospy.is_shutdown():
	    ...
	    gps_data.status.status = int(data[6])
            ...
            tail_pub.publish(gps_data)


if __name__ == "__main__":
    rospy.init_node('tail_gps_driver')
    rate = rospy.Rate(5)  # Hz

    tail_pub = rospy.Publisher('gps_tail', NavSatFix,queue_size=1)

    # 4800 baud for USB GPS
    tail = serial.Serial('/dev/ttyACM1', 115200, timeout = 2)  # 4800 baud for USB GPS

    gps_tail = NavSatFix()
    gps_driver(gps_tail, tail, '$GNGGA')
